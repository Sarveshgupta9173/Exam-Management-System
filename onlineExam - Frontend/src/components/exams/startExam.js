...
const StartExam = (props) => {
  ...
  useEffect(() => {
    window.onblur = () => {
      ...
      window.alert(
        `Tab changing and Refreshing not allowed. ${
          2 - malpractice
        } time more and you'll be disqualified`
      );
  ...
  function updateAfterTestcomplete() {
    ...
    const updateUrl = `${HOST_URL}/user-exam-login/update-user-exam-login`;
    axios
      .put(updateUrl, objToPost)
      .then((response) => {
    ...
    axios
      .put(updateExamStatusUrl, updateStatusObj)
      .then((response) => {
        console.log(response.data);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Countdown
            date={parseInt(sessionStorage.getItem("started_time")) + 300000}
            onComplete={testAutoComplete}
          />
        </div>
        ...
        >
            ...
            {allQuestions.map((element, index) => {
              ...
              return (
                <Button
                  ...
                  value={i}
                  onClick={handleClick}
                  id={element.question.question_id}
                  color={
          ...
          <DisplayQuestions
            ...
            buttonId={currentIndex}
            examId={exam_id}
            studentData={student_data}
            examData={exam_data}
            totalQuestions={allQuestions}
            setSelectedAnswers={setAnswerArray}
            selectedAnswerArray={answerArray}
          />
        </div>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              ...
              <Typography>
                You will get your results within few working days.
              </Typography>
            </DialogContentText>
          </DialogContent>
          <DialogActions>
