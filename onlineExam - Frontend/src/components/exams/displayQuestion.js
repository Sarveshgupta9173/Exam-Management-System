    ...
    const handleTextbox = (event)=>{
      ...
      const handleSubmit = (event)=>{
        ...
        setSelectedAnswers(prevArr => {
          ...
          const objToPost = {
            ...
            answer:(questionToDisplay.question_type === "MCQ")?selectedAnswer.split('-')[1] : selectedAnswer.trim() ,
            is_reviewed:false
          };
        console.log(objToPost);
          ...
          axios.post(submitUrl,objToPost)
                ...
                .catch((error)=>{
                  console.log(error);
                  setErrorMsg("Error ...Answer not submitted.")
                  setDanger("block");
                  setTimeout(() => {
                    setDanger("none");
                  }, 2000);
      ...
      const handleCopyPaste = (event)=>{
        ...
        axios.put(updateUrl,objToPost)
                ...
                .catch((error)=>{
                  console.log(error);
                  setErrorMsg("Error ocured while submitting exam.")
                  setDanger("block");
                  setTimeout(() => {
                    setDanger("none");
                  }, 3000);
                  ...
                  <div style={{margin:2}} >
                    <div style={{display:(textbox)?'none':'flex' , paddingLeft : 20}}>
                     ...
                     <div style={{ padding : '30px 0px'}}>
                        ...
                        <Button variant="contained" color="error" sx={{p:2,ml:5}} disabled={(buttonId==allQuestions.length)?true:false}
                         onClick={handleFinalSubmit}>Submit Exam</Button>
                     </div>
                  </div>
              </div>
